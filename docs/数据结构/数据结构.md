...
![反转链表示例 1：1→2→3→4→5 反转为 5→4→3→2→1](assets/反转链表-示例1.jpeg)
...
public:
    ListNode* reverseList(ListNode* head) 
    {
        ...
        ListNode* newHead = nullptr, take = head->next, breakNode = take->next;
        while (take) 
        {
            take->next = newHead; // 关键：反转指针
            newHead = take;
            take = breakNode;
               if(breakNode) breakNode=breakNode->next;
            //或写成breakNode = breakNode ? breakNode->next : nullptr;
        }
        return newHead;
    }
};
