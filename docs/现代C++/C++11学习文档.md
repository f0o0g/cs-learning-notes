> 本文档整理了 C++11 标准的新特性文章，共 36 篇，按稳定性与兼容性、易学易用性、通用性能提升、安全性、多线程五大主题分章，内容取自各篇原文。
...
原始字符串字面量的完整语法为 `R"delimiter(字符序列)delimiter"`。其中 `delimiter` 是可选的自定义分隔符；如果指定，左右两处必须完全一致。圆括号和双引号属于语法边界，不会成为字符串内容。
...
C++11 允许 `long long` 在不同平台上采用不同表示，但要求它至少（至少64位）能够表示从 `-9223372036854775807` 到 `9223372036854775807` 的范围。常用形式有两种：
...
与其他整数类型一样，可以通过 `<climits>`（C 兼容头为 `<limits.h>`）中的宏查询当前实现的取值范围。与 `long long` 相关的宏有三个：

  * `LLONG_MIN` \- 最小的long long值
  * `LLONG_MAX` \- 最大的long long 值
  * `ULLONG_MAX` \- 最大的 unsigned long long 值
...
上面的结果来自采用 64 位 `long long` 的实现。标准保证其取值宽度至少为 64 位，但不直接保证“至少占 8 字节”，因为一个字节的位数由实现定义；实际范围应以 `LLONG_MIN`、`LLONG_MAX` 和 `ULLONG_MAX` 为准。
...
### 1.3.1 C++98 标准的类成员初始化

在C++98中，支持了在类声明中使用等号 = 加初始值 的方式，来初始化类中静态成员常量 。这种声明方式我们也称之为”就地”声明。而`非静态成员变量的初始化则必须在构造函数中进行。`
...
在C++11中提供了专门的类型转换函数，程序员可以非常方便的使用它们进行数值类型和字符串类型之间的转换。
...
由于C++中的数值类型包括`整型`和`浮点型`，因此针对于不同的类型提供了不同的函数，通过调用这些函数可以将字符串类型转换为对应的数值类型。
```cpp
// 定义于头文件 <string>
...
[虚拟地址空间结构图](https://subingwen.cn/linux/file-descriptor/#1-2-%E5%88%86%E5%8C%BA)
...
通过输出的结果可以看出，nullptr 无法隐式转换为整型，但是可以隐式匹配指针类型。在 C++11 标准下，相比 NULL 和 0，使用 nullptr 初始化空指针可以令我们编写的程序更加健壮。
...
在介绍[gcc/g++工作流程的时候说过](https://subingwen.cn/linux/gcc/#2-gcc-%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B)，C++ 程序从编写完毕到执行分为四个阶段：`预处理`、 `编译`、`汇编`和`链接`4个阶段，得到可执行程序之后就可以运行了。需要额外强调的是，`常量表达式和非常量表达式的计算时机不同，非常量表达式只能在程序运行阶段计算出结果，但是常量表达式的计算往往发生在程序的编译阶段，这可以极大提高程序的执行效率`，因为表达式只需要在编译阶段计算一次，节省了每次程序运行时都需要计算一次的时间。
...
委托构造函数允许使用同一个类中的一个构造函数调用其他的构造函数，从而简化相关变量的初始化。下面举例说明：
```cpp
#include <iostream>
using namespace std;
...
```
  ...
  * 第4行中 `auto&&`表示一个整型的左值引用
  * 第5行中 `auto&&`表示一个整型的右值引用
...
代码中的 `return {9527, "华安"};` 会使用花括号中的实参直接初始化函数的返回对象。它并不等价于 `return (9527, "华安");`，后者使用的是逗号运算符。通过上面的几个例子可以看出，C++11 的列表初始化统一了多种对象的初始化方式，也让代码更加简洁清晰。
...
* `一旦定义了构造函数`，即使构造函数不包含参数，函数体里也没有任何的代码，`那么该构造函数也不再是"平凡"的`。
  ...
  class Test1 
  {
      Test1();	// 程序员定义的构造函数, 非默认构造
  };
  ```



关于析构函数也和上面列举的构造函数类似，一旦被定义就不平凡了。但是这也并非无药可救，使用[=default关键字](https://subingwen.cn/cpp/default-delete/)可以显式地声明默认的构造函数，从而使得类型恢复 “平凡化”。
...
有时程序员会忘记上面提到的规则，最常见的是声明了带参数的构造，如果还需要无参构造函数，这时候必须定义出不带参数的版本。不过通过编译器的提示，这样的问题通常会得到更正。但更为严重的问题是，一旦声明了自定义版本的构造函数，则有可能导致我们定义的类型不再是[POD类型](https://subingwen.cn/cpp/POD/)，我们便不再能够享受POD类型为我们带来的便利。

对于上面提到的这些，我们无需过度担心，因为C++11非常贴心地为我们提供了解决方案，就是使用=default 。

### 3.9.2 =default 和 =delete

在C++11标准中称`= default`修饰的函数为`显式默认【缺省】（explicit defaulted）函数`，而称`=delete`修饰的函数为`删除（deleted）函数或者显式删除函数`。
...
##### 3.9.2.1.3 **定义默认函数的注意事项:**

如果程序员对C++类提供的默认函数（上面提到的六个函数）进行了实现，那么可以通过 =default 将他们再次指定为默认函数，不能使用 =default 修饰这六个函数以外的函数。
```cpp
class Base
{
public:
...
=delete 表示显式删除，`显式删除可以避免用户使用一些不应该使用的类的成员函数`，使用这种方式可以有效地防止某些类型之间自动进行隐式类型转换产生的错误。下面举例说明：

##### 3.9.2.2.1 **禁止使用默认生成的函数**
```cpp
class Base
{
public:
...
```
  ...
  * 第5行：禁用拷贝构造函数
  * 第6行：禁用 = 进行对象复制
  * 第12行：拷贝构造函数已被显式删除，无法拷贝对象
  * 第13行：复制对象的赋值操作符重载函数已被显式删除，无法复制对象
...
`friend` 关键字用于声明类的友元。被授权的友元函数或友元类可以访问该类的 `public`、`protected` 和 `private` 成员。友元是类主动授予的、范围明确的访问权限；它会扩大可访问范围，因此应谨慎使用，但并不等同于完全取消封装。
...
C++98 已经能够声明友元模板；C++11 在这里新增的能力，是可以直接把模板类型参数声明为友元。实例化类模板时，如果该参数代表类类型，相应类型会成为友元；如果参数不是可声明为友元的类型，该声明会被忽略：
```cpp
class Tom;
...
```
  ...
  * 第5行：该行的代码有两处错误
    ...
    2. 强类型枚举不会隐式转换为整数，因此枚举值不能直接给 `int` 型变量赋值（其默认底层类型通常为 `int`，但仍需显式转换）。
  * 第6行：语法错误，将强类型枚举值作为整型使用，此处不会进行隐式类型转换
  * 第7行：语法正确，强类型枚举值在和整数比较之前做了强制类型转换。
  * 第11行：`打印的结果为4`，强类型枚举底层类型值默认为`int`，因此占用的内存是4个字节
  * 第12行：`打印的结果为1`，显式指定了强类型枚举的底层类型为 `char`，因此在该实现中占用 1 个字节。
...
在C++11之前我们使用的联合体是有局限性的，主要有以下三点：
  ...
  1. 不允许联合体拥有非[POD类型](https://subingwen.cn/cpp/POD/)的成员
  2. 不允许联合体拥有静态成员
  3. 不允许联合体拥有引用类型的成员
...
在 C++11标准中会默认删除一些非受限联合体的默认函数。比如，非受限联合体`有一个非 POD 的成员`，而该非 POD成员类型拥有 [非平凡的构造函数](https://subingwen.cn/cpp/POD/#2-%E2%80%9C%E5%B9%B3%E5%87%A1%E2%80%9D%E7%B1%BB%E5%9E%8B)，那么`非受限联合体的默认构造函数将被编译器删除`。其他的特殊成员函数，例如`默认拷贝构造函数、拷贝赋值操作符以及析构函数等，也将遵从此规则`。下面来举例说明：
```cpp
union Student
{
    int id;
...
上面代码中的非受限联合体`Student`中拥有一个非POD类型的成员`string name`，`string 类`中有非平凡构造函数，因此`Student`的构造函数被删除（通过警告信息可以得知它的析构函数也被删除了）导致对象无法被成功创建出来。解决这个问题的办法就是`由程序员自己为非受限联合体定义构造函数`，在定义构造函数的时候我们需要用到`定位放置 new`操作。

##### 3.12.2.2.1 **placement new**
...
在程序的第20行，使用定位放置的方式为指针`b`申请了一块内存，也就是说此时`指针 b`指向的内存地址和`变量 n`对应的内存地址是同一块（栈内存），而在`Base类`中`成员变量 number`的起始地址和`Base对象`的起始地址是相同的，所以打印出 number 的值为100也就是整型变量 n 的值。
...
> 某些安全软件可能会对包含底层内存操作的示例程序产生误报；遇到拦截时，应先检查源码和编译产物，不要直接关闭系统防护。

# 第四章 安全性

## 4.1 共享智能指针

C++ 不会自动回收通过动态分配获得的资源；如果所有权管理不当，就可能发生内存泄漏。智能指针（smart pointer）利用 RAII 将资源的生命周期绑定到对象生命周期，并在适当时机自动释放资源。`智能指针是存储指向动态分配（堆）对象指针的类，用于生存期的控制，能够确保在离开指针所在作用域时，自动地销毁动态分配的对象，防止内存泄露。智能指针的核心实现技术是引用计数，每使用它一次，内部引用计数加1，每析构一次内部的引用计数减1，减为0时，删除所指向的堆内存。`
需要注意，只有 `std::shared_ptr` 采用共享所有权和引用计数；`std::unique_ptr` 表示独占所有权，不使用引用计数，而 `std::weak_ptr` 只是对共享对象的非拥有观察者。
...
### 4.1.1 shared_ptr的初始化

共享智能指针是指多个智能指针可以同时管理同一块有效的内存，共享智能指针shared_ptr 是一个模板类，如果要进行初始化有三种方式：通过构造函数、std::make_shared辅助函数以及reset方法。共享智能指针对象初始化完毕之后就指向了要管理的那块堆内存，如果想要查看当前有多少个智能指针同时管理着这块内存可以使用共享智能指针提供的一个成员函数`use_count`，函数原型如下：
```cpp
...
`std::unique_ptr` 利用 RAII 管理资源，并以独占方式表达所有权：同一时刻通常只有一个 `unique_ptr` 拥有该资源。它不使用引用计数，不能复制，但可以通过移动操作转移所有权。
...
`std::weak_ptr` 与 `std::shared_ptr` 的控制块协作，但不拥有对象，也不会增加共享引用计数。它常用于观察共享对象、打破循环引用；需要访问对象时，应调用 `lock()` 临时取得 `shared_ptr`，并检查对象是否仍然存在。
...
`weak_ptr` 对象 `wp` 执行 `wp.reset();` 后变为空对象，不再观察任何资源，因此 `wp.expired()` 返回 `true`。

#### 4.3.1.3 返回管理this的shared_ptr
...
这个问题可以通过 `std::enable_shared_from_this<T>` 解决。类继承该模板后，可以调用 `shared_from_this()` 获得一个与现有 `shared_ptr` 共享同一控制块的 `shared_ptr`，从而避免为同一个裸指针创建多个互不相关的控制块。调用该函数时，对象必须已经由 `shared_ptr` 管理；否则在 C++11 中会抛出 `std::bad_weak_ptr`。
...
在测试程序中，共享智能指针`ap`、`bp`对`TA`、`TB`实例对象的引用计数变为2，`在共享智能指针离开作用域之后引用计数只能减为1`，这种情况下不会去删除智能指针管理的内存，导致类`TA`、`TB`的实例对象不能被析构，最终造成内存泄漏。通过使用`weak_ptr`可以解决这个问题，只要将类`TA`或者`TB`的任意一个成员改为`weak_ptr`，修改之后的代码如下：
```cpp
#include <iostream>
#include <memory>
using namespace std;
...
```
  ...
  * `Rep`：这是一个数值类型，表示时钟数（周期）的类型（默认为整型）。若 `Rep` 是浮点数，则 `duration` 能使用小数描述时钟周期的数目。
...
```
  ...
  * `d3(3.5)`时钟周期为1000秒，时钟周期数量只能用整型来表示，但是此处指定的是浮点数，因此语法错误
...
#### 5.1.3.2 steady_clock

如果使用时钟的目的不是获取当前系统时间，而是测量一段程序的耗时，那么 `system_clock` 通常并不合适，因为它可能随系统时间调整而发生跳变。C++11 提供的 `steady_clock` 是单调时钟，其时间点不会倒退，因此更适合测量时间间隔。

> 下面代码来自某个标准库实现，用于帮助理解。成员表示类型和具体周期均可能因平台而异；可移植代码只应依赖标准规定的接口以及 `is_steady == true`：
```cpp
...
> 通过以上源码可以了解到在`steady_clock`类中的一些细节信息：
  ...
  * `time_point`：该时钟对应的时间点类型，即 `chrono::time_point<steady_clock>`
...
```
  ...
  * [基于命名空间 this_thread 得到当前线程的线程ID](https://subingwen.cn/cpp/this_thread/#1-get-id)

上面的示例存在严重问题：两个 `std::thread` 对象离开作用域时仍处于可连接状态。`std::thread` 的析构函数不会自动等待或销毁所关联的执行线程，而是调用 `std::terminate()` 终止程序。因此，每个可连接的线程对象在析构前都必须调用 `join()` 或 `detach()`。
...
[基于命名空间 this_thread 让当前线程休眠](https://subingwen.cn/cpp/this_thread/#2-sleep-for)

#### 5.2.2.3 detach()

`detach()` 用于将执行线程与 `std::thread` 对象分离。分离后，线程可独立运行，并在任务结束时由运行时回收线程资源；原 `std::thread` 对象不再可连接。程序仍必须保证分离线程访问的对象在使用期间有效，并确保进程结束前完成必要工作，因为进程退出时尚未完成的分离线程也会被终止。函数原型如下：
```cpp
void detach();
```
...
注意：`detach()` 不会等待任务结束。分离后，该 `std::thread` 对象不再表示一个可连接线程，不能再调用 `join()`；`get_id()` 仍可调用，但会返回默认构造的线程 ID。由于分离线程的生命周期和错误处理更难管理，应优先采用 `join()` 或更高层的并发抽象。
...
通过以上`=`操作符的重载声明可以得知：
  ...
  * 如果`other`不是右值，禁止拷贝，该函数被显式删除（`=delete`），不可用
...
[C线程库函数的使用](https://subingwen.cn/linux/thread/)

## 5.3 线程命名空间this thread
...
[进程被创建后一共有五种状态](https://subingwen.cn/linux/process/#1-4-%E8%BF%9B%E7%A8%8B%E7%8A%B6%E6%80%81)
...
[chrono库中的时间段类 duration 的使用](https://subingwen.cn/cpp/chrono/#1-%E6%97%B6%E9%97%B4%E9%97%B4%E9%9A%94duration)
```cpp
...
[chrono库中的时间点类 time_point 的使用](https://subingwen.cn/cpp/chrono/#2-%E6%97%B6%E9%97%B4%E7%82%B9-time-point)
...
命名空间`this_thread`中提供了一个非常绅士的函数`yield()`，在线程中调用这个函数之后，处于运行态的线程会主动让出自己已经抢到的CPU时间片，最终变为就绪态，这样其他的线程就有更大的概率能够抢到CPU时间片了。使用这个函数的时候需要注意一点，线程调用了yield()之后会主动放弃CPU资源，但是这个变为就绪态的线程会马上参与到下一轮CPU的抢夺战中，不排除它能继续抢到CPU时间片的情况，这是概率问题。
```cpp
void yield() noexcept;
```
...
独占互斥锁对象有两种状态：`锁定`和`未锁定`。如果互斥锁是打开的，调用`lock()`函数的线程会得到互斥锁的所有权，并将其上锁，其他线程再调用该函数的时候由于得不到互斥锁的所有权，就会被`lock()`函数阻塞。当拥有互斥锁所有权的线程将互斥锁解锁，此时被`lock()`阻塞的线程解除阻塞，抢到互斥锁所有权的线程加锁并继续运行，没抢到互斥锁所有权的线程继续阻塞。
...
当互斥锁被锁定之后可以通过`unlock()`进行解锁，但是需要注意的是`只有拥有互斥锁所有权的线程也就是对互斥锁上锁的线程才能将其解锁，其他线程是没有权限做这件事情的。`该函数的函数原型如下：
```cpp
void unlock();
```
...
条件变量通常用于生产者和消费者模型，大致使用过程如下：
  ...
  2. 循环检查某个条件，如果条件不满足阻塞当前线程，否则线程继续向下执行
     * 产品数量达到上限时，生产者阻塞；否则生产者可以继续生产。
     * 产品数量为零时，消费者阻塞；否则消费者可以继续消费。
  3. 条件满足之后，可以调用`notify_one()`或者`notify_all()`唤醒一个或者所有被阻塞的线程
     * 消费者消费后唤醒被阻塞的生产者，使其继续生产。
     * 生产者生产后唤醒被阻塞的消费者，使其继续消费。

### 5.6.1 condition_variable
...
总结：以上两种条件变量各有特点。`condition_variable` 配合 `unique_lock<mutex>` 使用，等待期间能够自动释放并重新获取互斥锁；`condition_variable_any` 可以配合更广泛的、满足基本锁要求的锁类型，但通常有额外开销。实际使用时应优先选择 `condition_variable`，仅在确实需要其他锁类型时使用 `condition_variable_any`。

## 5.7 线程同步之原子变量atomic

C++11 提供了原子类型模板 `std::atomic<T>`。除标准库提供的布尔、整数和指针特化外，主模板还可用于满足要求的可平凡复制类型；因此不能简单地概括为“不支持浮点类型和复合类型”。不过，不同类型支持的成员运算不同，例如 C++11 中浮点原子类型没有 `fetch_add` 等专用算术成员。

原子操作的关键性质是：其他线程不会观察到该操作“只完成了一部分”的中间状态。具体实现可能使用处理器原子指令，也可能由标准库借助锁完成；它并不意味着暂停其他 CPU 核心，也不意味着操作期间绝不会发生线程调度。

原子类型可避免手工管理互斥锁，并为简单共享状态提供精确的内存顺序控制，但性能是否优于互斥锁取决于类型、平台、竞争程度和内存顺序。对复杂的不变量或多个相关对象进行一致更新时，仍然需要互斥锁等更高层同步机制。

原子类型也不保证一定无锁；可以使用 `is_lock_free()` 查询某个原子对象在当前实现中是否无锁。某些读改写操作可能通过 CAS 循环实现，在竞争激烈时仍可能反复重试。

C++11 为常用整数类型提供了原子别名，使单个变量的读取、写入和读改写操作更加方便。原子操作可以避免针对该原子对象的数据竞争，但它不能自动保护其他对象，也不能把多步业务逻辑合并为一个原子事务。是否需要互斥锁，应根据要维护的不变量和临界区范围决定，而不能仅凭使用了原子变量就一概取消。

CAS 是 Compare-and-Swap（比较并交换）的简称：它以原子方式比较对象当前值与期望值，并在相等时写入新值。C++ 通过 `compare_exchange_weak` 和 `compare_exchange_strong` 暴露这种语义；底层可能是一条或多条处理器指令，也可能由标准库以其他方式实现。
...
```
  ...
  * 构造函数③：使用`=delete`显式删除拷贝构造函数, 不允许进行对象之间的拷贝
...
模板类型T为整型 | T operator+= (T val) volatile noexcept; T operator+= (T val) noexcept; T operator-= (T val) volatile noexcept; T operator-= (T val) noexcept; T operator&= (T val) volatile noexcept; T operator&= (T val) noexcept; T operator|= (T val) volatile noexcept; T operator|= (T val) noexcept; T operator^= (T val) volatile noexcept; T operator^= (T val) noexcept;  
---|---  
...
**atomic_int8_t**(C++11)(可选) | [`std::atomic<std::int8_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint8_t**(C++11)(可选) | [`std::atomic<std::uint8_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int16_t**(C++11)(可选) | [`std::atomic<std::int16_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint16_t**(C++11)(可选) | [`std::atomic<std::uint16_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int32_t**(C++11)(可选) | [`std::atomic<std::int32_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint32_t**(C++11)(可选) | [`std::atomic<std::uint32_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int64_t**(C++11)(可选) | [`std::atomic<std::int64_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint64_t**(C++11)(可选) | [`std::atomic<std::uint64_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int_least8_t**(C++11) | [`std::atomic<std::int_least8_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint_least8_t**(C++11) | [`std::atomic<std::uint_least8_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int_least16_t**(C++11) | [`std::atomic<std::int_least16_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint_least16_t**(C++11) | [`std::atomic<std::uint_least16_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int_least32_t**(C++11) | [`std::atomic<std::int_least32_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint_least32_t**(C++11) | [`std::atomic<std::uint_least32_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int_least64_t**(C++11) | [`std::atomic<std::int_least64_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint_least64_t**(C++11) | [`std::atomic<std::uint_least64_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int_fast8_t**(C++11) | [`std::atomic<std::int_fast8_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint_fast8_t**(C++11) | [`std::atomic<std::uint_fast8_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int_fast16_t**(C++11) | [`std::atomic<std::int_fast16_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint_fast16_t**(C++11) | [`std::atomic<std::uint_fast16_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int_fast32_t**(C++11) | [`std::atomic<std::int_fast32_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint_fast32_t**(C++11) | [`std::atomic<std::uint_fast32_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_int_fast64_t**(C++11) | [`std::atomic<std::int_fast64_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uint_fast64_t**(C++11) | [`std::atomic<std::uint_fast64_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_intptr_t**(C++11)(可选) | [`std::atomic<std::intptr_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uintptr_t**(C++11)(可选) | [`std::atomic<std::uintptr_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_size_t**(C++11) | [`std::atomic<std::size_t>`](http://zh.cppreference.com/w/cpp/types/size_t)  
**atomic_ptrdiff_t**(C++11) | [`std::atomic<std::ptrdiff_t>`](http://zh.cppreference.com/w/cpp/types/ptrdiff_t)  
**atomic_intmax_t**(C++11) | [`std::atomic<std::intmax_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
**atomic_uintmax_t**(C++11) | [`std::atomic<std::uintmax_t>`](http://zh.cppreference.com/w/cpp/types/integer)  
...
多线程程序中的任务通常是异步执行的。如果想在一个线程中取得另一个异步任务的结果，可以使用 C++11 提供的 `std::future`。它通常与 `std::promise`、`std::packaged_task` 或 `std::async` 配合使用。下面先介绍该类的常用 API：
...
一般情况下使用`=`进行赋值操作就进行对象的拷贝，但是`future`对象不可用复制，因此会根据实际情况进行处理：
  ...
  * 如果`other`是非右值，不允许进行对象之间的拷贝（`该函数被显式删除禁止使用`）
...
当前线程调用 `wait()` 后会阻塞到共享状态就绪；调用 `wait_for()` 最多等待指定时长，超时并不代表异步任务已经完成。这里的任务不一定运行在子线程中，例如延迟任务可能在调用 `get()` 或 `wait()` 的线程中执行。
...
常量 | 解释  
---|---  
[`future_status::deferred`](https://zh.cppreference.com/w/cpp/thread/future_status) | 任务采用延迟执行策略，尚未开始执行  
[`future_status::ready`](https://zh.cppreference.com/w/cpp/thread/future_status) | 共享状态已就绪，可以取得结果或异常  
[`future_status::timeout`](https://zh.cppreference.com/w/cpp/thread/future_status) | 在指定等待时长内共享状态尚未就绪  
...
存储要传出的 `value` 值，并立即让状态就绪，这样数据被传出其他线程就可以得到这个数据了。重载的第四个函数是为`promise<void>`类型的对象准备的。
```cpp
...
### 5.8.3 std::packaged_task

`std::packaged_task`类包装了一个可调用对象包装器类对象（可调用对象包装器包装的是可调用对象，可调用对象都可以作为函数来使用），[恶补一下可调用对象和可调用对象包装器](https://subingwen.cn/cpp/bind/#1-%E5%8F%AF%E8%B0%83%E7%94%A8%E5%AF%B9%E8%B1%A1)
...
`std::async` 可以启动或延迟执行一个任务，并返回与该任务共享状态关联的 `future`。是否创建新线程取决于执行策略：`std::launch::async` 要求异步执行，`std::launch::deferred` 会把任务延迟到等待或取值时在调用线程中执行；使用默认策略时由实现选择。需要结果时调用 `future::get()`，只等待完成时可调用 `wait()` 或 `wait_for()`。函数原型如下：
```cpp
// 定义于头文件 <future>
// ①
template< class Function, class... Args>
...
相关函数介绍:
  ...
  * `addTask(function<void()> f)`：将任务添加到任务队列，并通知一个线程有新任务到来。
    ...
    * 线程的任务函数是一个可调用对象，类型为 `function<void()>`，即返回值类型为 `void`，无参数
